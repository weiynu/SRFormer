...
class Refinement(nn.Module):
    def __init__(self, rate1=0.1, rate2=1.0, regularization=None):
        ...
        # 改进3: 可学习的rate参数
        self.rate1 = torch.tensor(rate1)
        self.rate2 = torch.tensor(rate2)

        self.gate_linear=nn.Linear(1,1)

    def forward(self, input, output, network_1, network_2,):
        B, T, N, D = input.shape
        # 改进4: 添加批量归一化
        bn = nn.BatchNorm2d(N).to(DEVICE)
        
        input_std = torch.std(input.detach()).to(DEVICE)
        scale = torch.tensor(float(input_std * 5.0)).to(DEVICE)
        ...
        # 改进8: 更稳定的损失计算
        output=output.reshape(y_noise.shape)
        loss_eva=nn.HuberLoss()
        loss=loss_eva(y_noise, output)
    ...
    def get_attention(self,ratio):
        ...
        # 增加门控机制
        gate=torch.sigmoid(self.gate_linear(ratio))
        gated_attention=attention_weights*gate

        # 确保注意力权重和为1
        # attention_sum = torch.sum(attention_weights, dim=-1, keepdim=True)
        attention_sum = torch.sum(gated_attention, dim=-1, keepdim=True)
...
def attention_refinement(model, refiner, dataloader, network_1, network_2, refiner_optimizer, refiner_scheduler, optimizer, scheduler):
    ...
    for x,y in dataloader:
        ...
        for r in range(30):
            ...
            refine_loss, ratio = refiner(embeddings, y, network_1, network_2)
